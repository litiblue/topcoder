...
class BearPlaysDiv2:
    def __init__(self):
        self.v = defaultdict(lambda: defaultdict(lambda: defaultdict(bool)))

    def is_possible(self, A, B, C):
        if self.v[A][B][C]:
            return
        self.v[A][B][C] = True
    ...
    def equalPiles(self, A, B, C):
        #A, B, C = 283, 433, 208
        return 'possible' if self.is_possible(A, B, C) else 'impossible'
