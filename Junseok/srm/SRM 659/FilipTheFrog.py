...
class FilipTheFrog:
    def countReachableIslands(self, positions, L):
        first_num = positions[0]
        positions = sorted(positions)

        for idx, num in enumerate(positions):
            if num == first_num:
                first_idx = idx
                break
        ...
        # left
        for idx in xrange(first_idx, 0, -1):
            if positions[idx] - positions[idx-1] <= L:
                cnt1 += 1
            else:
                break

        # right
        for idx in xrange(first_idx, len(positions)-1):
            if positions[idx+1] - positions[idx] <= L:
                cnt2 += 1
            else:
                break
